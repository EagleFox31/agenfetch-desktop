...
function Home() {
  ...
  useEffect(() => {
    document.title = 'AgenFetch Desktop — YouTube arrive chez toi, sans cloud';
    const description = 'AgenFetch Desktop est une application Windows locale et simple pour télécharger des vidéos YouTube avec yt-dlp.';
    ...
    if (ogTitle) ogTitle.setAttribute('content', 'AgenFetch Desktop — sans cloud, sans compte.');
  }, []);
      ...
      <main id="top">
        <section className="hero-grid relative mx-auto grid max-w-[1240px] items-center gap-12 px-5 pb-20 pt-12 lg:grid-cols-[.92fr_1.08fr] lg:px-8 lg:pb-28 lg:pt-20" aria-labelledby="hero-title">
          <div className="relative z-10">
            ...
            <h1 id="hero-title" className="display-font reveal reveal-delay-1 mt-5 max-w-[620px] text-[clamp(2.15rem,8.4vw,5.65rem)] font-bold leading-[.96] tracking-[-.075em] text-[#282d46]">YouTube arrive chez toi.<br /><span className="text-[#176d64]">Sans cloud, sans compte.</span></h1>
            <p className="reveal reveal-delay-2 mt-6 max-w-[490px] text-[16px] leading-7 text-[#5f6370] sm:text-[17px]">Une interface propre pour yt-dlp, installée sur ton PC Windows. Tu colles un lien, tu choisis, tu récupères — sans configuration Node, yt-dlp ou FFmpeg à faire à la main.</p>
          ...
          <div className="reveal reveal-delay-2 relative z-10 lg:pl-4">
            <div className="absolute -right-4 -top-8 hidden rounded-full border border-[#d5cec0] bg-[#f8f4ea] px-3 py-2 font-mono text-[10px] text-[#176d64] shadow-[0_8px_18px_rgba(44,50,71,.08)] lg:block" style={{ animation: 'drift 5s ease-in-out infinite' }}>100% local</div>
            <Parallax speed={0.07} className="hero-float hero-float-one hidden lg:block"><span className="hero-float-icon"><Download size={13} /></span><span><strong>MP4 · 1080p</strong><small>Prêt à enregistrer</small></span></Parallax>
            <Parallax speed={-0.05} className="hero-float hero-float-two hidden lg:flex"><span className="hero-float-dot" /><span><strong>File séquentielle</strong><small>2 éléments · local</small></span></Parallax>
            <AppWindow />
        ...
        <section className="story-scene story-problem relative mx-auto max-w-[1240px] scroll-mt-8 px-5 pb-24 lg:px-8 lg:pb-32" aria-labelledby="problem-title">
          <Reveal className="story-scene-grid">
            <div className="story-copy">
              ...
              <p className="mt-5 max-w-[390px] text-[15px] leading-6 text-[#6e7079]">Il est perdu dans un onglet, une playlist à faire défiler, une adresse que tu ne reverras peut-être pas. AgenFetch transforme ce moment fugace en quelque chose de simple et local.</p>
              <div className="browser-fragment mt-8" data-testid="card-browser-fragment">
                ...
                <div className="browser-fragment-body"><div className="browser-play"><Play size={13} fill="currentColor" /></div><div><strong>La vidéo que tu veux garder</strong><small>Un lien parmi tous les autres</small></div><ArrowRight size={15} /></div>
              </div>
            </div>
        ...
        <section id="workflow" className="mx-auto max-w-[1240px] scroll-mt-8 px-5 pb-24 lg:px-8 lg:pb-32" aria-labelledby="workflow-title">
          <div className="grid gap-12 lg:grid-cols-[.65fr_1.35fr]">
            ...
            <div className="grid gap-8 md:grid-cols-3">
              {[{ n: '01', title: 'Colle un lien', text: 'Vidéo, Short, live ou playlist. AgenFetch lit le lien et affiche un aperçu avant de commencer.' }, { n: '02', title: 'Choisis ton format', text: 'MP4, MKV ou MP3. Sélectionne la qualité et les sous-titres FR, EN ou toutes les langues.' }, { n: '03', title: 'Récupère localement', text: 'La file séquentielle travaille proprement. Ton fichier est enregistré dans le dossier choisi.' }].map((step) => <div className="feature-tile" key={step.n} data-testid={`card-step-${step.n}`}><span className="step-number">{step.n}</span><h3 className="display-font mt-5 text-[20px] font-bold tracking-[-.04em] text-[#282d46]">{step.title}</h3><p className="mt-3 text-[13px] leading-5">{step.text}</p></div>)}
            </div>
          </div>
        </section>
        ...
        <section id="features" className="scroll-mt-8 bg-[#e9e3d7] py-24 lg:py-32" aria-labelledby="features-title">
          <div className="mx-auto max-w-[1240px] px-5 lg:px-8">
            <div className="flex flex-col justify-between gap-7 md:flex-row md:items-end"><div><div className="eyebrow">Ce que tu peux faire</div><h2 id="features-title" className="display-font mt-5 max-w-[550px] text-4xl font-bold leading-[1.02] tracking-[-.06em] text-[#282d46] sm:text-5xl">Un outil sérieux pour des usages simples.</h2></div><p className="max-w-[300px] text-[13px] leading-5 text-[#6e7079]">Pensé pour les vidéos que tu veux vraiment garder sous la main — pas pour collectionner des réglages.</p></div>
        ...
        <section id="trust" className="scroll-mt-8 py-24 lg:py-32" aria-labelledby="trust-title">
          <div className="mx-auto grid max-w-[1240px] gap-6 px-5 lg:grid-cols-[1.1fr_.9fr] lg:px-8">
            ...
            <div className="flex flex-col justify-between rounded-xl border border-[#d5cec0] bg-[#f7f3ea] p-8 sm:p-10"><div><div className="eyebrow">Avant d’installer</div><h3 className="display-font mt-5 text-3xl font-bold leading-tight tracking-[-.05em] text-[#282d46]">Une beta honnête.</h3><p className="mt-4 text-[14px] leading-6 text-[#6e7079]">Windows SmartScreen peut signaler que l’application est non signée. C’est attendu pour cette beta 0.2.1 distribuée directement depuis GitHub — pas un abonnement caché, pas un installateur opaque.</p></div><div className="mt-10 border-t border-[#d5cec0] pt-5"><p className="font-mono text-[10px] uppercase tracking-[.1em] text-[#7b7f87]">À utiliser pour</p><p className="mt-3 text-[13px] leading-5 text-[#555a68]">Les contenus autorisés, tes propres vidéos ou les œuvres libres de droits. Respecte les conditions de la plateforme et les droits des créateurs.</p></div></div>
          </div>
        </section>
